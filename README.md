...
- **If macOS blocks RoamRun's local-network access, the device always looks "away".** Every probe to this Wi-Fi then fails at once, and RoamRun goes on bridging (and advertising) a device that is sitting right next to it; over the mesh VPN everything else keeps working, so nothing else gives it away. From 0.1.14 RoamRun says so in the window, the activity log, `roamrun status` and `roamrun doctor`. Allow RoamRun in System Settings › Privacy & Security › Local Network. If the switch is already on, the permission is stuck — changing the app can do that, as 0.1.12's new bundle id did — and the app has to be reinstalled. The only reinstall confirmed to clear it was `brew uninstall --zap --cask roamrun` followed by `brew install --cask mh-mobile/tap/roamrun`; **`--zap` also deletes your saved devices**, so copy `~/Library/Application Support/RoamRun/profiles.json` aside first and put it back **before you open RoamRun again** (it writes its own list back over the file once it has one). Stop bridges started with `roamrun up -d` first too: one left running keeps its own denied state and goes on reporting it. A plain reinstall without `--zap` is untested ([#23](https://github.com/mh-mobile/RoamRun/issues/23))
